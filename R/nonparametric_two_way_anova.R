nonparametric_two_way_anova = function(project_ID = "negative mode_68410298_1517597840246",
                                       expression_id = "expression_data_68410298_1517597840246_csv",
                                       sample_info_id = "sample_info_68410298_1517597840246_csv",
                         group_name1 = "treatment",
                         group_name2 =  "time",
                         levels1 = c("JControl", "JLPS","QControl","QLPS"),
                         levels2 = c("2","6", "24"),
                         post_hoc_test_type = "pairwise_nonparametric_t_test_with_no_correction",
  ...
  core = nonparametric_two_way_anova_core(e,group1, group2,levels1, levels2,
  ...
  filename = "two way ANOVA with nonparamtric post hoc tests"
  ...
  for(i in 4:ncol(out)){
    if(colnames(out)[i] == "F_score"){
      ...
      text = paste0(text, "| `F_score_1` | The F statistic used to calculate p values of `ANOVA_p_value` |  |\n")
    }else if(colnames(out)[i] == "ANOVA_p_value"){
      text = paste0(text, "| `ANOVA_p_value` | the p value of ANOVA for testing the main effect of ",group_name2," | ",sum(out[[i]]<0.05, na.rm = T)," (",signif(sum(out[[i]]<0.05, na.rm = T)/nrow(out), 4)*100,"%) |\n")
      ...
      text = paste0(text, "| `",colnames(out)[i],"` | The F statistic used to calculate p values of `ANOVA_p_value @",tail(strsplit(colnames(out)[i],"@")[[1]],1),"` |  |\n")
    }else if(colnames(out)[i] %in% paste0("ANOVA_p_value @", levels1)){
      text = paste0(text, "| `",colnames(out)[i],"` | the p value of ANOVA for testing the simple main effect of ",group_name2," at level `",tail(strsplit(colnames(out)[i],"@")[[1]],1),"` | ",sum(out[[i]]<0.05, na.rm = T)," (",signif(sum(out[[i]]<0.05, na.rm = T)/nrow(out), 4)*100,"%) |\n")
  ...
  report = paste0(report, "| Column Name | Description | Number (%) of significant |\n| :--- | :--- | :--- |\n",text,"\nTable: ",filename,"\n\n")
  report = paste0(report, "The two-way nonparametric ANOVA analysis result is saved in the *",filename,"/nonparametric two-way ANOVA.xlsx*.")
  ...
  for(i in 1:length(levels2)){
    ...
    if("ANOVA_p_value" %in% colnames(out)[index]){
