nonparametric_anova = function(project_ID = "a4_68410298_1514765751212",
  ...
  core = nonparametric_anova_core(e,group_indexes,post_hoc_test_type)
  ...
  filename = "Kruskal–Wallis test"

  report = paste0("# ",filename,"\n\nThe statistical significance was evaluated by one-way nonparametric ANOVA, Kruskal–Wallis H test, with `",post_hoc_test_type,"` as post hoc test procedure on the _",group_name," (",paste0(levels,collapse = ", "),")_. To overcome the multiple comparison problem, we adopted Benjamini-Hochberg false discovery rate (FDR) correction on nonparametric ANOVA _p_ values. As a result, we found that there were ",sum(out[[2]]<0.05,na.rm = T)," (",signif(sum(out[[2]]<0.05,na.rm = T)/nrow(out),4)*100,"%) compounds had a nonparametric ANOVA _p_ value < 0.05, and after FDR correction, there were ",sum(out[[3]]<0.05,na.rm = T)," (",signif(sum(out[[3]]<0.05,na.rm = T)/nrow(out),4)*100,"%) compounds significant. A detailed explanation of each column is following, \n\n")

  text = ""
  for(i in 1:ncol(out)){
    if(colnames(out)[i] == "ANOVA_p_value"){
      text = paste0(text, "| `ANOVA_p_value` | the p value of nonparametric ANOVA (Kruskal–Wallis H test) for testing the effect of ",group_name," | ",sum(out[[i]]<0.05, na.rm = T)," (",signif(sum(out[[i]]<0.05, na.rm = T)/nrow(out), 4)*100,"%) |\n")
  ...
  report = paste0(report, "The nonparametric ANOVA analysis result is saved in the *",filename,"/ANOVA.xlsx*.")
