two_way_repeated_anova_core = function(e,group1, group2,levels1, levels2,id_index,
                                    main_effect_post_hoc_test_type = "pairwise_paired_t_test_with_fdr",
                                    simplemain_effect_post_hoc_test_type = "pairwise_paired_t_test_with_fdr"
){
  ...
  if(length(group1_indexes) == 2){
    ...
    anova1_core = repeated_anova_core(e, group1_indexes,id_index,sphericity_correction = T, post_hoc_test_type = main_effect_post_hoc_test_type)
  }
  # main effect 2
  if(length(group2_indexes) == 2){
    anova2_core = paired_t_test_core(e, group2_indexes, direction = "two.sided", id_index = id)
  }else{
    anova2_core = repeated_anova_core(e, group2_indexes,id_index,sphericity_correction = T, post_hoc_test_type = main_effect_post_hoc_test_type)
  }
  ...
  for(i in 1:length(group2_indexes)){
    ...
    if(length(group1_indexes)==2){
      ...
      simple_main_effect_1[[i]] = repeated_anova_core(e., group1_indexes., id.,sphericity_correction = T, post_hoc_test_type = simplemain_effect_post_hoc_test_type)
      colnames(simple_main_effect_1[[i]]) = paste0(names(simple_main_effect_1[[i]]), " @", names(group2_indexes[i]))
    }
  }
  ...
  for(i in 1:length(group1_indexes)){
    ...
    if(length(group2_indexes)==2){
      simple_main_effect_2[[i]] = paired_t_test_core(e., group2_indexes., direction = "two.sided", id_index = id.)
      colnames(simple_main_effect_2[[i]]) = paste0(names(simple_main_effect_2[[i]]), " @", names(group1_indexes[i]))
    }else{
      simple_main_effect_2[[i]] = repeated_anova_core(e., group2_indexes., id.,sphericity_correction = T, post_hoc_test_type = simplemain_effect_post_hoc_test_type)
      colnames(simple_main_effect_2[[i]]) = paste0(names(simple_main_effect_2[[i]]), " @", names(group1_indexes[i]))
    }
  }
