nonparametric_repeated_anova = function(project_ID = "A_68410298_1516162594176",
                          expression_id = "expression_data_68410298_1516162594176_csv",
                          sample_info_id = "sample_info_68410298_1516162594176_csv",
                          sample_ID_name = "mx sample id",
                          group_name = "treatment",
                          levels = c("MeOH-frz", "ACN.IPA-frz","MeOH.CHCl3-frz","MeOH.CHCl3-lyph"),
                          # sphericity_correction = TRUE,
                          post_hoc_test_type = "pairwise_nonparametric_paired_t_test_with_no_correction",
  ...
  core = nonparametric_repeated_anova_core(e,group_indexes,id_index,post_hoc_test_type)
  ...
  filename = "Friedman test"

  report = paste0("# ",filename,"\n\nThe statistical significance was evaluated by one-way nonparametric repeated ANOVA, Friedman test, _(paired by ",sample_ID_name,")_ with `",post_hoc_test_type,"` as post hoc test procedure on the _",group_name," (",paste0(levels,collapse = ", "),")_. To overcome the multiple comparison problem, we adopted Benjamini-Hochberg false discovery rate (FDR) correction on nonparametric repeated ANOVA _p_ values. As a result, we found that there were ",sum(out[[2]]<0.05,na.rm = T)," (",signif(sum(out[[2]]<0.05,na.rm = T)/nrow(out),4)*100,"%) compounds had a nonparametric ANOVA _p_ value < 0.05, and after FDR correction, there were ",sum(out[[3]]<0.05,na.rm = T)," (",signif(sum(out[[3]]<0.05,na.rm = T)/nrow(out),4)*100,"%) compounds significant. A detailed explanation of each column is following, \n\n")

  text = ""
  for(i in 1:ncol(out)){
    if(colnames(out)[i] == "repeated_ANOVA_p_value"){
      text = paste0(text, "| `repeated_ANOVA_p_value` | the p value of nonparametric repeated ANOVA (Friedman test) for testing the effect of ",group_name," | ",sum(out[[i]]<0.05, na.rm = T)," (",signif(sum(out[[i]]<0.05, na.rm = T)/nrow(out), 4)*100,"%) |\n")
  ...
  report = paste0(report, "The nonparametric repeated ANOVA analysis result is saved in the *",filename,"/ANOVA.xlsx*.")
