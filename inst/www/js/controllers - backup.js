  ...
  p.data.pop();
    ...
    .controller("bimo_scatter_plot_controller", function($scope,$mdDialog, $rootScope, $http, $timeout, $mdToast, get_filename_service, get_parent_service){
      ...
      var db = new PouchDB('http://slfan:metabolomics@chemrichdb2.fiehnlab.ucdavis.edu/db/abib');
        ...
        $scope.$apply(function(){
          $scope.expression_id_options = expression_id_options
          $scope.$watch("expression_id",function(newValue, oldValue){
            if(newValue !== oldValue){
          ...
          $scope.upload_file = function(){
            ...
            $scope.submit_data_setting = function(){
              ...
              /*if($scope.group_data_into_tarces){
                group  = group_data.data.map(d => d[$scope.group_data_column])
              }else{
                group = Array(group_data.data.length).fill('data')
              }*/
              if($scope.group_data_into_tarces){
                group = [];
              ...
              var req = ocpu.call("calculate_ellipse",{
                ...
                session.getObject(function(obj){
                  ...
                  $scope.$apply(function(){
                    ...
                    time = 1
                    plot_data = []
                    var group_levels = group.filter(unique)
                    var xaxis_range_min = Math.min(Math.min(...obj.ellipse.map(function(val){return val.map(v => v[0])}).map(v => Math.min(...v))) * 0.9,Math.min(...obj.ellipse.map(function(val){return val.map(v => v[0])}).map(v => Math.min(...v))) * 1.1)
                    var xaxis_range_max = Math.max(Math.max(...obj.ellipse.map(function(val){return val.map(v => v[0])}).map(v => Math.max(...v))) * 0.9,Math.max(...obj.ellipse.map(function(val){return val.map(v => v[0])}).map(v => Math.max(...v))) * 1.1)
                    var yaxis_range_min = Math.min(Math.min(...obj.ellipse.map(function(val){return val.map(v => v[1])}).map(v => Math.min(...v))) * 0.9,Math.min(...obj.ellipse.map(function(val){return val.map(v => v[1])}).map(v => Math.min(...v))) * 1.1)
                    var yaxis_range_max = Math.max(Math.max(...obj.ellipse.map(function(val){return val.map(v => v[1])}).map(v => Math.max(...v))) * 0.9,Math.max(...obj.ellipse.map(function(val){return val.map(v => v[1])}).map(v => Math.max(...v))) * 1.1)
                    for(var i=0;i<group_levels.length;i++){
                      var index = getAllIndexes(group, group_levels[i])
                      plot_data.push({
                        x: index.map(i => x[i]),
                        y: index.map(i => y[i]),
                        type: 'scatter', // set
                    ...
                    plot_layout = {
                          width:saved_plot_layout.width? saved_plot_layout.width:700*time,
                          height:saved_plot_layout.height? saved_plot_layout.height:450*time,
                          margin_open:saved_plot_layout.margin_open? saved_plot_layout.margin_open:false,
                          margin:{
                            l:saved_plot_layout.margin? saved_plot_layout.margin.l:80*time,
                            r:saved_plot_layout.margin? saved_plot_layout.margin.r:80*time,
                            t:saved_plot_layout.margin? saved_plot_layout.margin.t:100*time,
                            b:saved_plot_layout.margin? saved_plot_layout.margin.b:80*time,
                            pad:saved_plot_layout.margin? saved_plot_layout.margin.pad:0*time
                          },
                          paper_bgcolor:saved_plot_layout.paper_bgcolor? saved_plot_layout.paper_bgcolor:"#ffffff",
                          plot_bgcolor:saved_plot_layout.plot_bgcolor? saved_plot_layout.plot_bgcolor:"#ffffff",
                          showlegend:saved_plot_layout.showlegend? saved_plot_layout.showlegend:true,
                          title:'Scatter Plot',
                          titlefont:{
                            family:saved_plot_layout.titlefont? saved_plot_layout.titlefont.family:"Arial",
                            size:saved_plot_layout.titlefont? saved_plot_layout.titlefont.size:15*time,
                            color:saved_plot_layout.titlefont? saved_plot_layout.titlefont.color:"#000000"
                          },
                          hovermode:saved_plot_layout.hovermode? saved_plot_layout.hovermode:"closest",//set
                          xaxis:{
                            visible:saved_plot_layout.xaxis? saved_plot_layout.xaxis.visible:true,
                            //color:"#000000",
                            title:$scope.x_axis,
                            titlefont:{
                              family:saved_plot_layout.xaxis? saved_plot_layout.xaxis.titlefont.family:"Arial",
                              size:saved_plot_layout.xaxis? saved_plot_layout.xaxis.titlefont.size:12 * time,
                              color:saved_plot_layout.xaxis? saved_plot_layout.xaxis.titlefont.color:"#000000"
                            },
                            autorange:saved_plot_layout.xaxis? saved_plot_layout.xaxis.autorange:false,// set
                            type:saved_plot_layout.xaxis? saved_plot_layout.xaxis.type:"linear", //set
                            xaxis_range_min:xaxis_range_min,
                            xaxis_range_max:xaxis_range_max,
                            range:[xaxis_range_min,xaxis_range_max],
                            tickmode:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickmode:"auto",
                            nticks:saved_plot_layout.xaxis? saved_plot_layout.xaxis.nticks:6,
                            tickvals:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickvals:"[]",
                            ticktext:saved_plot_layout.xaxis? saved_plot_layout.xaxis.ticktext:"[]",
                            ticks:saved_plot_layout.xaxis? saved_plot_layout.xaxis.ticks:'inside',
                            ticklen:saved_plot_layout.xaxis? saved_plot_layout.xaxis.ticklen:5*time,
                            tickwidth:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickwidth:1*time,
                            tickcolor:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickcolor:"#000000",
                            showticklabels:saved_plot_layout.xaxis? saved_plot_layout.xaxis.showticklabels:true,
                            tickfont:{
                              family:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickfont.family:"Arial",
                              size:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickfont.size:12*time,
                              color:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickfont.color:"#000000",
                            },
                            tickangle:saved_plot_layout.xaxis? saved_plot_layout.xaxis.tickangle:0,
                            showline:saved_plot_layout.xaxis? saved_plot_layout.xaxis.showline:true,
                            linecolor:saved_plot_layout.xaxis? saved_plot_layout.xaxis.linecolor:"#000000",
                            linewidth:saved_plot_layout.xaxis? saved_plot_layout.xaxis.linewidth:1*time,
                            showgrid:saved_plot_layout.xaxis? saved_plot_layout.xaxis.showgrid:false,
                            gridcolor:saved_plot_layout.xaxis? saved_plot_layout.xaxis.gridcolor:"#eeeeee",
                            gridwidth:saved_plot_layout.xaxis? saved_plot_layout.xaxis.gridwidth:1*time,
                            zeroline:saved_plot_layout.xaxis? saved_plot_layout.xaxis.zeroline:false,
                            zerolinecolor:saved_plot_layout.xaxis? saved_plot_layout.xaxis.zerolinecolor:'#444444',
                            zerolinewidth:saved_plot_layout.xaxis? saved_plot_layout.xaxis.zerolinewidth:1*time,
                            side:saved_plot_layout.xaxis? saved_plot_layout.xaxis.side:"bottom"
                          },
                          yaxis:{
                            visible:saved_plot_layout.yaxis? saved_plot_layout.yaxis.visible:true,
                            //color:"#000000",
                            title:$scope.y_axis,
                            titlefont:{
                              family:saved_plot_layout.titlefont? saved_plot_layout.yaxis.titlefont.family:"Arial",
                              size:saved_plot_layout.titlefont? saved_plot_layout.yaxis.titlefont.size:12 * time,
                              color:saved_plot_layout.titlefont? saved_plot_layout.yaxis.titlefont.color:"#000000"
                            },
                            autorange:saved_plot_layout.yaxis? saved_plot_layout.yaxis.autorange:false,// set
                            type:saved_plot_layout.yaxis? saved_plot_layout.yaxis.type:"linear", //set
                            yaxis_range_min:yaxis_range_min,
                            yaxis_range_max:yaxis_range_max,
                            range:[yaxis_range_min,yaxis_range_max],
                            tickmode:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickmode:"auto",
                            nticks:saved_plot_layout.yaxis? saved_plot_layout.yaxis.nticks:6,
                            tickvals:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickvals:"[]",
                            ticktext:saved_plot_layout.yaxis? saved_plot_layout.yaxis.ticktext:"[]",
                            ticks:saved_plot_layout.yaxis? saved_plot_layout.yaxis.ticks:'inside',
                            ticklen:saved_plot_layout.yaxis? saved_plot_layout.yaxis.ticklen:5*time,
                            tickwidth:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickwidth:1*time,
                            tickcolor:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickcolor:"#000000",
                            showticklabels:saved_plot_layout.yaxis? saved_plot_layout.yaxis.showticklabels:true,
                            tickfont:{
                              family:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickfont.family:"Arial",
                              size:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickfont.size:12*time,
                              color:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickfont.color:"#000000",
                            },
                            tickangle:saved_plot_layout.yaxis? saved_plot_layout.yaxis.tickangle:0,
                            showline:saved_plot_layout.yaxis? saved_plot_layout.yaxis.showline:true,
                            linecolor:saved_plot_layout.yaxis? saved_plot_layout.yaxis.linecolor:"#000000",
                            linewidth:saved_plot_layout.yaxis? saved_plot_layout.yaxis.linewidth:1*time,
                            showgrid:saved_plot_layout.yaxis? saved_plot_layout.yaxis.showgrid:true,
                            gridcolor:saved_plot_layout.yaxis? saved_plot_layout.yaxis.gridcolor:"#eeeeee",
                            gridwidth:saved_plot_layout.yaxis? saved_plot_layout.yaxis.gridwidth:1*time,
                            zeroline:saved_plot_layout.yaxis? saved_plot_layout.yaxis.zeroline:false,
                            zerolinecolor:saved_plot_layout.yaxis? saved_plot_layout.yaxis.zerolinecolor:'#444444',
                            zerolinewidth:saved_plot_layout.yaxis? saved_plot_layout.yaxis.zerolinewidth:1*time,
                            side:saved_plot_layout.yaxis? saved_plot_layout.yaxis.side:"left"
                            }
                    }
                    ...
                    $scope.plot_data_ellipse_fillcolor_transformation = function(i){
                      plot_data_ellipse[i].fillcolor = hexToRgb($scope.plot_data_ellipse[i].plot_data_ellipse_fillcolor,$scope.plot_data_ellipse[i].plot_data_ellipse_fillcolor_opacity)
                    }
                    $scope.update_range = function(){
                      plot_layout.xaxis.range = [$scope.plot_layout.xaxis.xaxis_range_min, $scope.plot_layout.xaxis.xaxis_range_max]
                      plot_layout.yaxis.range = [$scope.plot_layout.yaxis.yaxis_range_min, $scope.plot_layout.yaxis.yaxis_range_max]
                    }
                    $scope.plot_data_ellipse = plot_data_ellipse
                    $scope.toggle_ellipse = function(i){
                      if(!plot_data_ellipse[i].show_ellipse){
                        plot_data_ellipse[i].line.width=0
                        plot_data_ellipse[i].fill="none"
                        plot_data_ellipse[i].showlegend=false
                      }else{
                        plot_data_ellipse[i].line.width=1
                        plot_data_ellipse[i].fill="toself"
                        plot_data_ellipse[i].showlegend=true
                      }
                    }
                    ...
                    $scope.save_attribute = function(){
                      ...
                      db_user_info.get(localStorage.getItem("username")).then(function(doc){
                        dd = doc
                        doc.saved_plot_data = plot_data
                        doc.saved_plot_layout = plot_layout
                        doc.saved_plot_data_ellipse = plot_data_ellipse
          ...
          $scope.$watch("plot_data",function(newValue, oldValue){
            if(newValue!==oldValue){
              ...
              //$scope.scatter_plot($scope.plot_data)
            }
          },true)
          ...
          $scope.scatter_plot = function(data,layout){
            ...
            .then(
                            function(gd)
                             {
                              Plotly.toImage(gd,{height:plot_layout.height,width:plot_layout.width})
                                 .then(
                                    function(url)
                                 {
                                   plot_url = url
            ...
            $scope.confirm_save_plot = function(ev){
              var d = new Date();
              var time_string = d.getTime().toString()
              $scope.saving = true
              var confirm = $mdDialog.prompt()
              .title('Give a name of the plot file.')
              .textContent('Please do not include any special characters.')
              .placeholder('input a name without special characters.')
              .ariaLabel('file name')
              .targetEvent(ev)
                ...
                console.log(result)
                  ...
                  var db = new PouchDB('http://slfan:metabolomics@chemrichdb2.fiehnlab.ucdavis.edu/db/abib');
                      db.get(localStorage.getItem("project_id_global"), {attachments: true}).then(function(doc){
                        ...
                        doc.tree_structure.push({
                          id:filename+"_68410298_"+time_string+"_png",
                          parent:filename+"_68410298_"+time_string,
                          text:filename+".png",
                          icon:"fa fa-file-picture-o",
                          source:{
                          ...
                          attname:filename+"_68410298_"+time_string+".png",
                          source_node_id:filename+"_68410298_"+time_string,
                          type:""
                        })

                        doc['_attachments'][filename+"_68410298_"+time_string+".png"] = {
                              content_type:"image/png",
